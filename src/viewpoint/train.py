import os
from argparse import ArgumentParser

import numpy as np
import polars as pl
import torch
from sklearn.metrics import classification_report, precision_recall_fscore_support
from torch import nn
from torch.utils.data import WeightedRandomSampler
from tqdm.auto import tqdm

from src.viewpoint.data import (
    ID2SCENE,
    ID2VIEWPOINT,
    compute_ce_class_weights,
    compute_class_weights,
    make_dataloader,
)
from src.viewpoint.model import ViewpointClassifier

TASK_TARGET_NAMES = {
    "scene": [ID2SCENE[i] for i in range(len(ID2SCENE))],
    "viewpoint": [ID2VIEWPOINT[i] for i in range(len(ID2VIEWPOINT))],
}


def train_epoch(model, criteria, optimizer, loaders, device, use_viewpoint=False):
    tasks = ["scene", "viewpoint"] if use_viewpoint else ["scene"]
    loss_stat = {"train": 0, "val": 0}
    acc_stat = {"train": {t: 0 for t in tasks}, "val": {t: 0 for t in tasks}}
    f1_stat = {"train": {t: 0 for t in tasks}, "val": {t: 0 for t in tasks}}
    for phase in ["train", "val"]:
        if phase == "train":
            model.train()
        else:
            model.eval()

        running_loss = 0
        running_correct = {t: 0 for t in tasks}
        n_samples = 0
        all_preds = {t: [] for t in tasks}
        all_labels = {t: [] for t in tasks}

        for batch in tqdm(
            loaders[phase], leave=False, desc=f"Running {phase} phase.."
        ):
            if use_viewpoint:
                inp, scene_lbl, viewpoint_lbl = batch
                viewpoint_lbl = viewpoint_lbl.to(device)
            else:
                inp, scene_lbl = batch
            inp = inp.to(device)
            scene_lbl = scene_lbl.to(device)

            optimizer.zero_grad()
            with torch.set_grad_enabled(phase == "train"):
                scene_out, viewpoint_out = model(inp)
                scene_preds = torch.argmax(scene_out, dim=1)
                loss = criteria["scene"](scene_out, scene_lbl)
                if use_viewpoint:
                    viewpoint_preds = torch.argmax(viewpoint_out, dim=1)
                    loss = loss + criteria["viewpoint"](viewpoint_out, viewpoint_lbl)

                if phase == "train":
                    loss.backward()
                    optimizer.step()

            running_loss += loss.item()
            running_correct["scene"] += (scene_preds == scene_lbl).sum().item()
            n_samples += scene_lbl.size(0)
            all_preds["scene"].append(scene_preds.detach().cpu())
            all_labels["scene"].append(scene_lbl.detach().cpu())
            if use_viewpoint:
                running_correct["viewpoint"] += (viewpoint_preds == viewpoint_lbl).sum().item()
                all_preds["viewpoint"].append(viewpoint_preds.detach().cpu())
                all_labels["viewpoint"].append(viewpoint_lbl.detach().cpu())

        epoch_loss = running_loss / len(loaders[phase])
        loss_stat[phase] = epoch_loss
        print(f"{phase.capitalize()} Loss: {epoch_loss:.4f}")

        for t in tasks:
            preds = torch.cat(all_preds[t]).numpy()
            labels = torch.cat(all_labels[t]).numpy()
            epoch_acc = running_correct[t] / n_samples
            epoch_prec, epoch_rec, epoch_f1, _ = precision_recall_fscore_support(
                labels, preds, average="macro", zero_division=0
            )
            print(
                f"  [{t}] {phase.capitalize()} Acc: {epoch_acc:.4f} | "
                f"Macro-P: {epoch_prec:.4f} | Macro-R: {epoch_rec:.4f} | "
                f"Macro-F1: {epoch_f1:.4f}"
            )
            acc_stat[phase][t] = epoch_acc
            f1_stat[phase][t] = epoch_f1

    return loss_stat, acc_stat, f1_stat


@torch.no_grad()
def run_evaluate(model, criteria, loader, device, use_viewpoint=False):
    model.eval()

    tasks = ["scene", "viewpoint"] if use_viewpoint else ["scene"]
    running_loss = 0
    running_correct = {t: 0 for t in tasks}
    n_samples = 0
    all_preds = {t: [] for t in tasks}
    all_labels = {t: [] for t in tasks}

    for batch in tqdm(loader, leave=False, desc="Running evaluation.."):
        if use_viewpoint:
            inp, scene_lbl, viewpoint_lbl = batch
            viewpoint_lbl = viewpoint_lbl.to(device)
        else:
            inp, scene_lbl = batch
        inp = inp.to(device)
        scene_lbl = scene_lbl.to(device)

        scene_out, viewpoint_out = model(inp)
        scene_preds = torch.argmax(scene_out, dim=1)
        loss = criteria["scene"](scene_out, scene_lbl)
        if use_viewpoint:
            viewpoint_preds = torch.argmax(viewpoint_out, dim=1)
            loss = loss + criteria["viewpoint"](viewpoint_out, viewpoint_lbl)

        running_loss += loss.item()
        running_correct["scene"] += (scene_preds == scene_lbl).sum().item()
        n_samples += scene_lbl.size(0)
        all_preds["scene"].append(scene_preds.cpu())
        all_labels["scene"].append(scene_lbl.cpu())
        if use_viewpoint:
            running_correct["viewpoint"] += (viewpoint_preds == viewpoint_lbl).sum().item()
            all_preds["viewpoint"].append(viewpoint_preds.cpu())
            all_labels["viewpoint"].append(viewpoint_lbl.cpu())

    test_loss = running_loss / len(loader)
    print(f"Test Loss: {test_loss:.4f}")

    results = {"loss": test_loss}
    for t in tasks:
        preds = torch.cat(all_preds[t]).numpy()
        labels = torch.cat(all_labels[t]).numpy()
        test_acc = running_correct[t] / n_samples
        precision, recall, f1, _ = precision_recall_fscore_support(
            labels, preds, average="macro", zero_division=0
        )
        print(
            f"  [{t}] Acc: {test_acc:.4f} | Precision: {precision:.4f} | "
            f"Recall: {recall:.4f} | F1: {f1:.4f}"
        )
        print(f"  [{t}] classification report:")
        print(
            classification_report(
                labels,
                preds,
                labels=list(range(len(TASK_TARGET_NAMES[t]))),
                target_names=TASK_TARGET_NAMES[t],
                zero_division=0,
            )
        )
        results[t] = {
            "accuracy": test_acc,
            "precision": precision,
            "recall": recall,
            "f1": f1,
        }

    return results


def main(args):
    ### CONFIG ###
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)

    device = (
        "mps"
        if torch.mps.is_available()
        else "cuda"
        if torch.cuda.is_available()
        else "cpu"
    )
    print(f"device: {device}")

    if not os.path.exists(args.ckpt_dir):
        os.makedirs(args.ckpt_dir)

    ### DATASET ###
    print("loading train/val/test data and making dataloaders")
    use_viewpoint = args.use_viewpoint
    train = pl.read_csv(args.train_path)
    val = pl.read_csv(args.val_path)
    test = pl.read_csv(args.test_path)
    ### IMBALANCE MITIGATION ###
    strategy = args.balance
    print(f"class-imbalance mitigation strategy: {strategy}")
    train_sampler = None
    ce_weights = None
    if strategy == "oversample":
        _, sample_weights = compute_class_weights(train, use_viewpoint=use_viewpoint)
        train_sampler = WeightedRandomSampler(
            sample_weights, num_samples=len(sample_weights), replacement=True
        )
    elif strategy == "weighted-loss":
        ce_weights = {
            t: torch.tensor(w, dtype=torch.float, device=device)
            for t, w in compute_ce_class_weights(
                train, use_viewpoint=use_viewpoint
            ).items()
        }

    trainloader = make_dataloader(
        train,
        args.base_img_path,
        args.batch_size,
        split="train",
        sampler=train_sampler,
        use_viewpoint=use_viewpoint,
    )
    valloader = make_dataloader(
        val, args.base_img_path, args.batch_size, split="val", use_viewpoint=use_viewpoint
    )
    testloader = make_dataloader(
        test, args.base_img_path, args.batch_size, split="test", use_viewpoint=use_viewpoint
    )

    ### MODEL ###
    model = ViewpointClassifier(use_viewpoint=use_viewpoint).to(device)
    criteria = {
        "scene": nn.CrossEntropyLoss(
            weight=ce_weights["scene"] if ce_weights else None
        )
    }
    if use_viewpoint:
        criteria["viewpoint"] = nn.CrossEntropyLoss(
            weight=ce_weights["viewpoint"] if ce_weights else None
        )
    head_params = list(model.scene_head.parameters())
    if use_viewpoint:
        head_params += list(model.viewpoint_head.parameters())
    optimizer = torch.optim.AdamW(head_params, lr=args.lr)

    ### TRAIN ###
    best_f1 = -np.inf
    for ep in range(args.epochs):
        print(f"Epoch {ep+1}/{args.epochs}")
        _, _, f1s = train_epoch(
            model,
            criteria,
            optimizer,
            loaders={"train": trainloader, "val": valloader},
            device=device,
            use_viewpoint=use_viewpoint,
        )

        if use_viewpoint:
            val_f1 = (f1s["val"]["scene"] + f1s["val"]["viewpoint"]) / 2
        else:
            val_f1 = f1s["val"]["scene"]
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(
                model.state_dict(), os.path.join(args.ckpt_dir, "viewpoint_best.pt")
            )
        print("-" * 10)

    ### INFERENCE ###
    print("running inference on test set with best checkpoint")
    model.load_state_dict(
        torch.load(os.path.join(args.ckpt_dir, "viewpoint_best.pt"), map_location=device)
    )
    run_evaluate(model, criteria, testloader, device, use_viewpoint=use_viewpoint)


if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("--train-path", required=True)
    parser.add_argument("--val-path", required=True)
    parser.add_argument("--test-path", required=True)
    parser.add_argument(
        "--base-img-path", default="/home/affahrizain/projects/datasets/geotir"
    )
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--ckpt-dir", default="checkpoints/viewpoint/")
    parser.add_argument(
        "--balance",
        choices=["oversample", "weighted-loss", "none"],
        default="none",
        help=(
            "Class-imbalance mitigation: 'oversample' (WeightedRandomSampler), "
            "'weighted-loss' (class-weighted CrossEntropyLoss), or 'none' "
            "(default) to disable both."
        ),
    )
    parser.add_argument(
        "--use-viewpoint",
        action="store_true",
        default=False,
        help="Also train the ground/aerial viewpoint head (off by default).",
    )

    args = parser.parse_args()
    main(args)