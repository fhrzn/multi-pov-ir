...
class ViewpointClassifier(nn.Module):
    ...
    def __init__(self, weights: str = "IMAGENET1K_V1", use_viewpoint: bool = False):
        super().__init__()
        ...
        self.viewpoint_head = ClassifierHead(in_features, 2) if use_viewpoint else None

    def forward(self, x: torch.Tensor):
        feats = self.base_model(x)
        if self.use_viewpoint:
            return self.scene_head(feats), self.viewpoint_head(feats)
        return self.scene_head(feats), None
